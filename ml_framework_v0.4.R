...
if (Sys.info()["nodename"] == 'BOHR') {
    folder.lib <- 'G:/R_data/MyRLibrary'
}
if (Sys.info()["nodename"] == 'EINSTEIN') {
    folder.lib <- 'C:/R_data/MyRLibrary'
}

source(paste(folder.lib, 'cleanly.R', sep = '/'))
source(paste(folder.lib, 'strfns.R', sep = '/'))
source(paste(folder.lib, 'mlfns.R', sep = '/'))
source(paste(folder.lib, 'numfns.R', sep = '/'))

folder.dat <- 'C:/Users/Allen/OneDrive/Programming and Analysis/R/MyRApplications/ML Practice'
...
titanic3 <- addPredictors(titanicwe)
...
# Here is where we could transform the data using a custom function (defined in this script or in a
